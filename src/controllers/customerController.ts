...
export const getAllCustomers = async (
  ...
  try {
    ...
    const countResult = database.prepare('SELECT COUNT(*) as total FROM customers').get() as { total: number };
    const total = countResult.total;

    // Obtener clientes paginados
    const customers = database
      .prepare('SELECT * FROM customers ORDER BY customer_id DESC LIMIT ? OFFSET ?')
...
export const deleteCustomer = async (
  ...
  try {
    ...
    const activeRentals = database
      .prepare('SELECT COUNT(*) as count FROM rentals WHERE customer_id = ? AND status = "rented"')
